    ...
    const [stats, recentOrders, topProducts, lowStock] = await Promise.all([
      pool.request().query(`
        SELECT
          ...
          (SELECT COUNT(*) FROM dbo.Products WHERE is_active = 1) AS total_products,
          ...
          (SELECT ISNULL(SUM(total_amount), 0) FROM dbo.Orders WHERE MONTH(created_at) = MONTH(GETDATE()) AND YEAR(created_at) = YEAR(GETDATE())) AS revenue_this_month
      `),
      pool.request().query(`
...
export async function updateProduct(req, res) {
  try {
    ...
    let query = 'UPDATE dbo.Products SET updated_at = GETDATE()'
    ...
    res.status(500).json({ success: false, message: 'Failed to UPDATE dbo.product' })
  }
}
...
export async function getAllUsers(req, res) {
  try {
    ...
    const result = await pool.request()
      .query('SELECT user_id, first_name, last_name, email, phone, role, is_active, created_at, last_login FROM dbo.Users ORDER BY created_at DESC')
...
export async function updateUserStatus(req, res) {
  try {
    ...
    await pool.request()
      ...
      .query('UPDATE dbo.Users SET is_active = @is_active WHERE user_id = @id')
